...
freshPuzzle s = Puzzle s (map toNothing s) "" where
  toNothing c = Nothing
...
gameOver :: Puzzle -> IO ()
gameOver (Puzzle wordToGuess _ guessed) = if (lengthWrongGuessed wordToGuess guessed) > 7 
  then do 
    putStrLn "You lose!"
    ...
    return ()
  where
    lengthWrongGuessed :: String -> String -> Integer
    lengthWrongGuessed _ [] = 0
    lengthWrongGuessed s (g:gs) = if elem g s then lengthWrongGuessed s gs else 1 + lengthWrongGuessed s gs
